...
基于 PyQt6 的剪贴板增强工具:常驻后台记录你复制过的**文本、图片、表情包、文件**,
随时点一下就能把历史内容重新粘贴出来。
...
| 操作 | 效果 |
| --- | --- |
| 复制任何内容(Ctrl+C) | 自动进入历史列表(文本 / 图片·表情包 / 文件) |
| **单击** 条目 / 回车 | 放回系统剪贴板 → 到目标应用 Ctrl+V 即可粘贴(解决历史内容无法粘贴的问题) |
| **双击** 条目 | 复制并自动粘贴到上一个活动窗口,本窗口保持打开(依赖 `xdotool`) |
| **拖拽条目到其他应用** | 直接完成粘贴(文本拖进编辑器、图片拖进聊天框/文件夹) |
| **列表内上下拖动** | 调整条目位置(排序会保存) |
| **右键** 条目 | 复制 / 添加到常用 / 修改字体颜色 / 恢复默认 / 移到顶部 / 删除 |
| **📋 历史 / ⭐ 常用** 页签 | 历史页自动记录复制内容;常用页存放固定条目,右键历史条目「⭐ 添加到常用」收藏,或在常用页点「➕ 添加」手动新建文本,单击/双击/拖拽用法与历史页相同 |
| 搜索框(Ctrl+F) | 按内容过滤历史 |
| Delete 键 | 删除选中条目 |
| 📌 最前(工具栏最右) | 窗口保持在最前,方便拖拽;条目置顶用右键菜单的"移到顶部" |
| ⚙ 设置 | 全局快捷键(默认 **Ctrl+Alt+V** 呼出/隐藏窗口)、历史上限、字体大小、界面语言(跟随系统/中文/English,切换即时生效)、双击自动粘贴开关、开机自启 |
| Ctrl+Alt+V(可改) | 在任何应用里一键呼出/隐藏本窗口;**正在输入时窗口停靠在输入位置旁边,否则停靠屏幕右上角**(Windows 按真实文本光标定位;Linux/macOS 以鼠标位置近似) |
...
- 从外部把文字/图片/文件**拖进**窗口,拖到哪个页面就存进哪个列表。
- 常用页不受"历史条数上限"限制,不会被自动清理。
...
- `xdotool`(可选):双击"复制并粘贴"需要它 —— `sudo apt install xdotool`。
...
| 双击自动粘贴 | xdotool 切窗口,窗口保持打开 | 隐藏窗口后模拟 Ctrl+V | 隐藏窗口后模拟 Cmd+V |
